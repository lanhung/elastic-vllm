...
on the Azure coding trace, mean SLO attainment is 0.941 for KV-util, 0.929 for
HPA, and 0.901 for a candidate policy that counts parked programs. Prefix
...
We park a 16k-token target, wait $\tau\in\{0,8,32\}$~s, introduce
$N\in\{0,4,8,16,24\}$ distinct 4k-token neighbors, and probe the target.
We normalize probe TTFT between the same-trial hit and miss to estimate retained
prefix fraction. All 45 trials completed. At $N\le8$, retention is essentially
one; at $N=16$, it is 0.645; and at $N=24$, it is approximately zero. The
three delay curves nearly overlap. In this range, allocation pressure rather
than wall-clock age determines survival.
...
19,607 token-equivalents/s and decode weight 94.59 relative to prefill. Maximum
...
$(T,\tau)=(4,4)$, 89.5\% at $(8,8)$, and 92.9\% at $(16,8)$.
At those points, respectively 16.2\%, 26.2\%, and 24.3\% of post-warm-up
...
\sys 0.695. At $(16,30)$, \sys attains 1.000 and HPA 0.994. At $(64,8)$,
KV-util and \sys both attain 1.000 while HPA is 0.792. Such crossovers are
...
At $T=8,\tau=8$, HPA recomputes 2.19M tokens, of which only 0.324M are
attributed to scale-in. KEDA, KV-util, and \sys recompute 9.96M, 6.88M, and
11.17M tokens respectively; their scale-in components are only 0.514M,
0.176M, and 0.361M. Thus routine cache pressure is the dominant source of lost
...
\sys attain 1.000, 0.782, 0.521, and 0.711 at $(T,\tau)=(8,8)$.
At 180~s they attain 0.605, 0.674, 0.399, and 0.644. The original monotone
...
KV-util 0.921, and \sys 0.974; KEDA falls to 0.280. On the coding trace at the
same point, the values are 1.000, 0.881, and 0.711 for the first, third, and
...
1.000/0.881/0.711. At batch 16 the same values are
1.000/0.930/0.942; at batch 64, 0.998/0.796/0.596. Sweeping capacity from
31k to 500k also changes the ordering, and changing \sys's target from 0.5 to
0.9 moves its attainment from 0.845 to 0.633. The candidate is sensitive to
