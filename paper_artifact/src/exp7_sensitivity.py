...
HORIZON = 2400.0
WARMUP = 300.0
T_VALUES, TAU, N0 = (8, 64), 8.0, 5


def run(df, hw: HW, policy, turns: int) -> object:
    progs = build_programs(df, turns_per_program=turns, think_mean_s=TAU,
...
def main():
    ...
    for turns in T_VALUES:
        ...
        for kv, lab in [(31_000, "31k"), (62_000, "62k"),
            ...
            for mk in (lambda: HpaGpuUtil(), lambda: KedaQueue(),
                       lambda: KvUtil(),
                       lambda: ParkAware(max_batch=hw.max_batch),
                       lambda: PressureAwareAdmission()):
                r = run(df, hw, mk(), turns)
                rows.append(dict(T=turns, sweep="kv_capacity", value=kv,
                                 label=lab, policy=r.policy, slo=r.slo_attain,
                                 goodput=r.goodput, gpu_s=r.gpu_seconds))
            print(f"  T={turns} A kv={kv:,}")
        ...
        for b in (8, 16, 32, 64):
            ...
            for mk in (lambda: HpaGpuUtil(), lambda: KedaQueue(),
                       ...
                       lambda: PressureAwareAdmission()):
                r = run(df, hw, mk(), turns)
                rows.append(dict(T=turns, sweep="max_batch", value=b,
                                 label=str(b), policy=r.policy,
                                 slo=r.slo_attain, goodput=r.goodput,
                                 gpu_s=r.gpu_seconds))
            print(f"  T={turns} B batch={b}")

        # --- C: ParkAware target --------------------------------------
        for th in (0.5, 0.6, 0.7, 0.8, 0.9):
            r = run(df, HW(), ParkAware(
                target=th, max_batch=HW().max_batch), turns)
            rows.append(dict(T=turns, sweep="target", value=th,
                             label=f"{th}", policy="park-aware",
                             slo=r.slo_attain, goodput=r.goodput,
                             gpu_s=r.gpu_seconds))
        print(f"  T={turns} C target sweep")
    ...
    for turns in T_VALUES:
        for s in d.sweep.unique():
            q = d[(d["T"] == turns) & (d.sweep == s)]
            print(f"=== T={turns} {s} (SLO attainment) ===")
            print(q.pivot_table(index="label", columns="policy",
