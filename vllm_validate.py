...
def kv_usage() -> float:
    m = vllm_metrics()
    for k in ("vllm:gpu_cache_usage_perc", "vllm:gpu_cache_usage"):
        if k in m:
            return m[k]
    return float("nan")
...
def make_prompt(n_tokens: int, tag: str) -> str:
    ...
    # roughly 1 token per short word for this crude filler
    body = " ".join(f"{tag}{i:06d}" for i in range(n_tokens))
...
def v2_parking(out: Path, n_sessions: int = 8, turns: int = 6,
    ...
    def run_session(sid: int):
        ...
        for k in range(turns):
            ...
            requests.post(f"{BASE}/v1/completions",
                          json={"model": MODEL, "prompt": p, "max_tokens": 32,
    ...
    for s in samples:
        s["t"] -= t_start
    _save(samples, out / "v2_parking_samples.csv")
    _save(phases, out / "v2_parking_phases.csv")

    # headline: what do the two signals read during a tool call?
    import pandas as pd
    sm = pd.DataFrame(samples); ph = pd.DataFrame(phases)
    park = ph[ph.phase == "park"]
    inpark = np.zeros(len(sm), dtype=bool)
    for _, r in park.iterrows():
        inpark |= ((sm.t >= r.t0) & (sm.t <= r.t1)).values
    # a moment is "all-parked" if every session is parked
    def fmt(mask, lab):
        g, r, w, k = (sm.gpu[mask].mean(), sm.running[mask].mean(),
                      sm.waiting[mask].mean(), sm.kv[mask].mean())
        gs = "n/a (vGPU)" if np.isnan(g) else f"{g:5.1f}%"
        print(f"  {lab:20s} GPU={gs:>11s}  running={r:5.2f}  "
              f"waiting={w:5.2f}  KV={k*100:5.1f}%")

    print("\n  --- the three signals the autoscalers read ---")
    fmt(inpark, "during tool calls")
    fmt(~inpark, "during compute")
    ...
    for k in ks:
        for rep in range(reps):
            ...
            with ThreadPoolExecutor(max_workers=k) as ex:
                list(ex.map(lambda p: requests.post(
                    f"{BASE}/v1/completions",
                    json={"model": MODEL, "prompt": p, "max_tokens": gen,
                          "temperature": 0.0}, timeout=600), prompts))
            dt = time.perf_counter() - t0
            rows.append(dict(k=k, rep=rep, wall_s=dt,
                             per_req_s=dt, agg_tok_s=k * gen / dt))
...
def v4_cold_start(out: Path, model: str | None = None, port: int = 8001,
    ...
    for rep in range(reps):
        ...
        print(f"  V4 rep={rep} cold_start={ready:.1f}s")
        time.sleep(5)
